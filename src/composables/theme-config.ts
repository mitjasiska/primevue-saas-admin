...
type Palette = {
  [shade in (typeof shades)[number]]: string;
};

type colorType = 'primary' | 'surface';

export function useThemeConfig() {
  const getPrimaryColorNames = () => {
    return ['noir'].concat(primaryColors.map((color) => color.name));
  };

  const getSurfaceNames = () => {
    return surfaces.map((color) => color.name);
  };

  const updateColors = (type: colorType, colorName: string) => {
    let selectedColor;
    const root = document.documentElement;

    // noir primary color is handled different
    if (colorName === 'noir') {
      state.primaryColor = colorName;
      root.classList.add('p-noir');
      applyNoir();
    } else {
      root.classList.remove('p-noir');

      if (type === 'primary' && colorName !== 'noir') {
        state.primaryColor = colorName;
        selectedColor = primaryColors.find((color) => color.name === colorName);
      } else if (type === 'surface') {
        state.surface = colorName;
  ...
  const applyNoir = () => {
    shades.forEach((inc) => {
      document.documentElement.style.setProperty(`--p-primary-${inc}`, `var(--p-surface-${inc})`);
    });
  };

  const applyTheme = (type: colorType, colors: Palette) => {
    shades.forEach((inc) => {
      document.documentElement.style.setProperty(`--p-${type}-${inc}`, colors[inc]);
    });
  };

  return { getPrimaryColorNames, getSurfaceNames, updateColors };
}
