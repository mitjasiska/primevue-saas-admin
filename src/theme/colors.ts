...
export type PrimaryColorNamesWithNoir = 'noir' | (typeof primaryColorNames)[number];

export type SurfaceColorNames = (typeof surfaceColorNames)[number];

type PrimaryColorPalette = {
  [color in PrimaryColorNames]: { [shade in PrimaryShades[number]]: string };
};

export type SurfaceColorPalette = {
  [color in SurfaceColorNames]: { [shade in SurfaceShades[number]]: string };
};
...
export const surfaceColorNames = [
  'slate',
  'gray',
  'zinc',
  'neutral',
...
// prettier-ignore
export const primaryColorPalette: PrimaryColorPalette = {
...
// prettier-ignore
export const surfaceColorPalette: SurfaceColorPalette= {
...
const getPreset = (color: PrimaryColorNamesWithNoir | SurfaceColorNames) => {
  if (color === 'noir') {
    return {
      semantic: {
        primary: {
        ...
        colorScheme: {
          light: {
            primary: {
              color: `{${color}.950}`,
              inverseColor: `#ffffff`,
              hoverColor: `{${color}.900}`,
              activeColor: `{${color}.800}`
            },
            highlight: {
              background: `{${color}.950}`,
              focusBackground: `{${color}.700}`,
              color: `#ffffff`,
              focusColor: `#ffffff`
            }
          },
          dark: {
            primary: {
              color: `{${color}.50}`,
              inverseColor: `{${color}.950}`,
              hoverColor: `{${color}.100}`,
              activeColor: `{${color}.200}`
            },
            highlight: {
...
export const updateTheme = (color: PrimaryColorNamesWithNoir | SurfaceColorNames) => {
  updatePreset(getPreset(color));
};
